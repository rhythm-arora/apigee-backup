...
def get_kvm_list_source(user_input):
    """
    Get the list of all KVM in the source environment of APIGEE ORG
    """
    app_data = []
    try:
        env = user_input['copykvmenv']
...
def get_kvm_details(user_input, kvm_list_source):
    """
    Get the values of all KVM from the source envirnment and store it in a list
    """
    kvm_details = []
    try:
        env = user_input['copykvmenv']
...
def create_kvm_destination(user_input, kvm_details):
    """
    Creating the KVM in destination environment of APIGEE
    """
    env = user_input['deploykvmenv']
    confirm = input("Confirm if you want to create KVM in {} env? Yes or No : ".format(env))
